  ...
  Stick.prototype = {
	show: function () {
	  this.renderer
		...
		.stroke( this.colors[ this.state ] )
		.polygon( 0, 0, BIG_R, 8 )
		.polygon( this.location.x, this.location.y, SMALL_R, 8 );
...
Asteroid.prototype = {
  update: function ( dt ) {
	...
	if ( this.location.x < this.radius ) {
	  ...
	  this.velocity.x *= -1;
	} else if ( this.location.x + this.radius > w ) {
	...
	if ( this.location.y < this.radius ) {
	  ...
	  this.velocity.y *= -1;
	} else if ( this.location.y + this.radius > h ) {
...
var update = function ( dt ) {
  ...
  if ( ui.hidden ) {
	...
	if ( shoots && time > threshold ) {
	  ...
	  bullets.push(
		[ ship.location.x, ship.location.y, ship.angle, ship.velocity.copy() ],
		[ ship.location.x, ship.location.y, ship.angle + pi * angle, ship.velocity.copy() ],
		[ ship.location.x, ship.location.y, ship.angle - pi * angle, ship.velocity.copy() ] );
  ...
  for ( i = bullets.length - 1; i >= 0; --i ) {
	bullet = bullets[ i ];
	bullet[ 0 ] += ( 750 * cos( bullet[ 2 ] ) + bullet[ 3 ].x ) * dt;
	bullet[ 1 ] += ( 750 * sin( bullet[ 2 ] ) + bullet[ 3 ].y ) * dt;
...
var render = function () {
  var shipX = ship.location.x,
	  shipY = ship.location.y,
	  i = asteroids.length - 1,
	  r = minimap.width * 0.5,
	  asteroid, x, y;
  ...
  for ( ; i >= 0; --i ) {
	...
	x = ( asteroid.location.x - shipX ) * 0.06;
	y = ( asteroid.location.y - shipY ) * 0.06;
